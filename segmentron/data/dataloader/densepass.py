...
#                     else:
#                         logging.info('cannot find the mask or image: %s, %s', imgpath, maskpath)
...
class DensePASSSegmentation(SegmentationDataset):
    ...
    def __init__(self, root='datasets/outdoor_8noji_image_raw_2025-02-26-14-06-25', split='val', mode=None, transform=None, **kwargs):
        super(DensePASSSegmentation, self).__init__(root, split, mode, transform, **kwargs)
        assert os.path.exists(self.root), "Please put dataset in {SEG_ROOT}/datasets/outdoor_8noji_image_raw_2025-02-26-14-06-25"
        self.images, self.mask_paths = _get_city_pairs(self.root, self.split)
        # self.crop_size = [400, 2048]  # for inference only
