...
class Stanford2d3dPan8Segmentation(SegmentationDataset):
    ...
    def __init__(self, root='datasets/raw_dkan_2025-08-08-18-23-13', split='val', mode=None, transform=None, **kwargs):
        super(Stanford2d3dPan8Segmentation, self).__init__(root, split, mode, transform, **kwargs)
        assert os.path.exists(root), "Please put the data in {SEG_ROOT}/datasets/raw_dkan_2025-08-08-18-23-13"
...
if __name__ == '__main__':
    ...
    input_transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize((.485, .456, .406), (.229, .224, .225)),])
     # Create Dataset
    trainset = Stanford2d3dPan8Segmentation(split='val', transform=input_transform)
