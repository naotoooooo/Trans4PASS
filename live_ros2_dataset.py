...
class _ImageBufferNode(Node):
    ...
    def __init__(self,
        ...
        # (img_np, stamp(Time), frame_id)
        self.img_buf: deque[Tuple[np.ndarray, Time, str]] = deque(maxlen=queue_size)
        # (stamp_ns, mask_np)
        self.msk_buf = deque(maxlen=queue_size)
    ...
    def _cb_image(self, msg: RosImage):
        try:
            ...
            stamp_msg: Time = msg.header.stamp
            frame_id: str = msg.header.frame_id
            with self.lock:
                self.img_buf.append((cv, stamp_msg, frame_id))
    ...
    def pop_image(self) -> Optional[Tuple[np.ndarray, Time, str]]:
        """Pop the oldest image. Returns (img_np, stamp(Time), frame_id) or None if no image yet."""
        with self.lock:
            if not self.img_buf:
                return None
            im, stamp_msg, frame_id = self.img_buf.popleft()
            return im, stamp_msg, frame_id
    ...
    def __iter__(self):
        ...
        while True:
            ...
            if popped is not None:
                im_np, stamp_msg, frame_id = popped
                ...
                # stamp_ns はログや可視化で使いたければここで作る
                # stamp_ns = stamp_msg.sec * 10**9 + stamp_msg.nanosec
                yield img_t, (msk_t if msk_t is not None else torch.empty(0, dtype=torch.long)), stamp_msg, frame_id
                end_by_timeout_at = None
            else:
                # no image yet
