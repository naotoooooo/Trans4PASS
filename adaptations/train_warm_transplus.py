...
from dataset.hachinoji_dataset import densepassDataSet, densepassTestDataSet
...
SOURCE_NAME = 'CS'
TARGET_NAME = 'outdoor_8noji_domaindata'
DATA_DIRECTORY = '/home/user/ws/src/Trans4PASS/datasets/cityscapes'
DATA_LIST_PATH = '/home/user/ws/src/Trans4PASS/adaptations/dataset/cityscapes_list/train.txt'
IGNORE_LABEL = 255
INPUT_SIZE = '1024,512'
DATA_DIRECTORY_TARGET = '/home/user/ws/src/Trans4PASS/datasets/outdoor_8noji_domaindata'
DATA_LIST_PATH_TARGET = '/home/user/ws/src/Trans4PASS/adaptations/dataset/8noji_list/train.txt'
DATA_LIST_PATH_TARGET_TEST = '/home/user/ws/src/Trans4PASS/adaptations/dataset/8noji_list/val.txt'
INPUT_SIZE_TARGET = '1280,320'
...
POWER = 0.9
RANDOM_SEED = 1234
RESTORE_FROM = '/home/user/ws/src/Trans4PASS/workdirs/cityscapes/trans4pass_plus_small_512x512/trans4pass_plus_small_512x512.pth'
SAVE_NUM_IMAGES = 2
...
def main():
    ...
    for i_iter in range(Iter, args.num_steps):
        ...
        if i_iter % args.save_pred_every == 0 and i_iter != 0:
            ...
            #######追記######
            logging.info('taking snapshot (skip validation) …')
            # モデルと Discriminator をそのまま保存
            torch.save(
                model.state_dict(),
                osp.join(args.snapshot_dir, f'iter_{i_iter}.pth')
            )
            torch.save(
                model_D.state_dict(),
                osp.join(args.snapshot_dir, f'iter_{i_iter}_D.pth')
            )
            ################
