...
class densepassDataSet(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, crop_size=(1280, 320),
    ...
    def __getitem__(self, index):
        ...
        if len(self.ssl_dir)>0:
            label = Image.open(osp.join(self.ssl_dir, name.replace('.png', '_labelIds.png')))
            if self.trans == 'resize':
                # resize
...
class densepassTestDataSet(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, crop_size=(1280, 320), mean=(128, 128, 128),
        ...
        for name in self.img_ids:
            img_file = osp.join(self.root, "leftImg8bit/%s/%s" % (self.set, name))
            lbname = name.replace(".png", "_labelIds.png")
...
if __name__ == '__main__':
    dst = densepassTestDataSet("datasets/outdoor_8noji_domaindata", 'dataset/8noji_list/val.txt', mean=(0,0,0))
